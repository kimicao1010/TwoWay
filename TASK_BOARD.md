...
| 日期 | 变更 |
|---|---|
| 2026-09-17 | **C6-6 R13 改版：⌘Q 确认改为窗体内提示 + 激活态限定，发布 v1.0.3（PRD v1.16）**：① 移除系统 `NSAlert` 确认（用户要求不弹窗）—— 主窗口**激活态**按 ⌘Q 在窗体中央浮出「按住 ⌘Q 键即可退出」深色小条（`QuitHintView`，2.5s 自动消失），提示期内**第二次 ⌘Q 即退出**；② 主窗口未激活 / 状态栏面板与 ⋯ 菜单的显式「退出」→ 直接退出（`quitImmediately()`），从不确认；③ **实测坑**：在 `NSEvent` 本地监听回调栈里同步调 `NSApplication.terminate` 会被吞掉（日志显示已调用、进程仍存活）→ 改为 `DispatchQueue.main.async` 延迟一个 runloop，并新增最小 `AppDelegate` 观测 `applicationShouldTerminate` 确实抵达（取证日志 `applicationShouldTerminate 抵达`）；④ 实现：`QuitGuard`（`@Observable` 状态机 + 依赖注入便于单测）+ `QuitGuardPolicy`（纯策略）+ `RootView` overlay 渲染提示。单测 209 → **213 全绿**（策略 3 + 状态机 5：首次提示 / 二次退出 / 未激活直接退 / 显式入口跳过）。版本 1.0.2 → **1.0.3** |
| 2026-09-17 | **C6-5 修复 ⌘W 失效 + 补 ⌘Q 退出确认，发布 v1.0.2（PRD v1.15）**：① ⌘W 失效是 v1.0.1 的回归 —— scene 改单实例 `Window` 后，Close 项从 File 菜单搬进 **`Window` 菜单**，而该菜单 **AppKit 懒加载**（探针实测启动期 `menus=5` 却无任何 ⌘W 项 → 补丁 `patched=0` 静默打空），且该项被 SwiftUI 判为 **disabled**，**禁用项会吞掉自己的快捷键**。修法：新增 `AppShortcutMonitor`（`NSEvent` 本地监听，在事件进入菜单系统前拦下 ⌘W/⌘Q），菜单补丁降级为 best-effort 并在菜单展开时重打；`CloseWindowMenu` 的匹配条件去掉 `action == performClose:`（scene 变更后不再命中）。② **⌘Q 二次确认**（R13，用户期望但**此前从未存在**——已核对代码、Demo、文档与 git 全部历史）：主窗口开启 → `NSAlert` 确认（**默认按钮为取消**，Esc 亦可），主窗口已关 → 直接退出；经 `@NSApplicationDelegateAdaptor` + `applicationShouldTerminate` 覆盖所有退出入口。③ 验证（全部**定向**：`CGEvent.postToPid` / AX，不用全局按键）：⌘W 关窗 ✅（多次）、窗口开启时 ⌘Q 弹确认 ✅、按「取消」应用存活且窗口保留 ✅；202 → **204 测试全绿**（新增 `QuitConfirmationPolicyTests`）。④ 已知限制（如实记录）：窗口关闭后 macOS 不再向应用派发按键（实测退出委托回调都不触发）→ 该状态退出请用状态栏面板「退出」；`Window ▸ Close` 菜单项在 SwiftUI 下仍灰显（键盘路径已由监听器保证）。⑤ 版本 1.0.1 → **1.0.2** |
| 2026-09-17 | **C6-4 修复状态栏重复建窗 + 发布 v1.0.1（PRD v1.14，用户反馈）**：现象「点状态栏 → 打开主窗口，每点一次多一个窗口，即使主窗口还开着」。根因：主窗口 scene 是 `WindowGroup`，而 `openWindow(id:)` 的语义就是每次新建（**阳性对照**：用 CGEvent 合成真实鼠标点击驱动状态栏面板，实测点 3 次得到 3 个 360×732 窗口、级联偏移 29px，与用户描述一致）。修法：① `WindowGroup` → 单实例 **`Window`**；② 新增 `MainWindowRegistry`（`WindowConfigurator` 登记主窗口）+ `MainWindowPolicy`（有窗口对象 → 前置/重新显示，无 → `openWindow`），面板「打开主窗口」走策略。③ **连带坑**：关窗走 AppKit `close()`（`CloseWindowMenu`），SwiftUI 不知场景已空 → 单实例 `Window` 的 `openWindow` 变成**空操作**（关闭态点击什么都不出现，属于本次改动引入的回归，已修）→ 登记表刻意**不在关闭时清空**，改用 `makeKeyAndOrderFront` 重新显示。验收（同一合成点击口径）：修复前 3 次 → 3 个窗口；修复后 3 次 → **恒为 1 个**（AX 口径计数、CG `onscreen=true`、最前台应用=2way）；关闭态（`--debug-close-window` 一次性关闭）点击 → 重开 1 个并保持。新增 `MainWindowPolicyTests` 4 例（含「已有窗口时绝不允许 openNew」护城河），测试 198 → **202 全绿**。新增调试项 `--debug-close-window`（回归关闭态；勾子刻意只关一次 —— 重新显示会再次触发 `onAppear`，每次关会把重开路径测成死循环）。版本 `MARKETING_VERSION` 1.0 → **1.0.1** |
