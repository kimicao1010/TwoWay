...
| 拖动手感（三条铁律） | ① 跟手位移**绝不动画**（动画会在每次落点变化时重新平滑这一行，而光标仍在移动 → 抖动）；② 跨格判定**必须带滞回**（否则光标停在边界 ±1px 就来回翻转，反复重触发让位动画）；③ `offsetY` 的读取要**收敛到被拖动那一行**（放在列表容器里读 → 鼠标每移动一像素整表重算 body） | v1.12 用户反馈「严重抖动」的根因就是这三条同时缺失。取证脚本：`--debug-drag-script`（带 ±3pt 边界抖动，自报落点序列/回翻数/tick 偏差） |
