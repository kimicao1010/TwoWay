...
| 拖动手感（铁律 1：坐标系） | **手势位移必须在稳定的命名坐标系测量**（`AccountListCoordinateSpace` 挂在滚动容器上），**绝不能用默认 `.local`** | ⚠️ v1.13 定位的真实根因：拖动行的位置由祖先 `.offset` 决定，而 `.local` 坐标系的原点就是**行自己被位移后的位置** → 「位移 = 光标位置 − 起点」里的光标位置被行自身位移抵消 → 行弹回 → 再跟手，每帧一次锯齿（实测：`startY` 恒定而 `locY` 在 11↔35 间跳，`dy` = -26,-2,-28,-4…）。换成命名坐标系后 `dy` 单调、零回退。**排查手段**：`--debug-drag-trace`（逐事件记录 start/location） |
| 拖动手感（铁律 2-4，v1.12 加固） | ② 跟手位移**绝不动画**；③ 跨格判定**必须带滞回**（±4pt）；④ `offsetY` 的读取要**收敛到被拖动那一行** | 这三条是**放大器**而非根因（曾误判）：动画会在落点变化时重新平滑行自身、无滞回时边界 ±1px 就翻转、在容器里读 offsetY 会让整表每像素重算。取证：`--debug-drag-script`（自报落点序列/回翻数/tick 偏差） |
...
| 主窗口必须单实例 | 主窗口用 **`Window`（单实例 scene）**，**不要用 `WindowGroup`**；「打开主窗口」走 `MainWindowPolicy` + `MainWindowRegistry`，不要直接 `openWindow` | `WindowGroup` 是为多窗口/多文档设计的，`openWindow(id:)` **每次调用都新建**窗口（实测点 3 次 → 3 个 360×732 窗口）。改单实例 `Window` 后还有一个**必须知道的坑**：本应用关窗走 AppKit `close()`，SwiftUI 不知道场景已空，此时 `openWindow(id:)` 是**空操作**（关窗后点状态栏什么都不出现）→ 登记表**刻意不在关闭时清空**，用 `makeKeyAndOrderFront` 重新显示 |
| ⌘W/⌘Q 不要依赖菜单项 | **⌘W 关窗、⌘Q 退出由 `AppShortcutMonitor`（`NSEvent` 本地监听）保证**；菜单项补丁只作 best-effort | 三个坑叠加：① D10 隐藏窗口按钮后 `Window ▸ Close` 被 SwiftUI 判为 **disabled**，而**禁用项吞掉自己的快捷键**；② 主窗口 scene 改单实例 `Window` 后 Close 项从 File 菜单搬进 **`Window` 菜单**，而该菜单 **AppKit 懒加载**（启动期 `menus=5` 却无任何 ⌘W 项 → 补丁静默打空）；③ 无窗口时 `Quit` 项同样不可用。菜单何时建立、SwiftUI 如何校验都不可控 → 键盘事件在进入菜单系统前拦截 |
| ⌘Q 防误触（R13） | **不要用系统 `NSAlert` 弹窗**：主窗口激活态按 ⌘Q → 在窗体内浮出 `QuitHintView`（2.5s 自动消失），提示期内第二次 ⌘Q 才退出；未激活态与显式点击（状态栏 / ⋯ 菜单「退出」）直接退出。真正的 `terminate` 必须**延迟一个 runloop**（`DispatchQueue.main.async`） | 两个实测坑：① 在 `NSEvent` 本地监听回调栈里同步调 `terminate` 会被**吞掉**（日志显示已调用、进程仍存活）；② `QuitGuard` 的状态与提示渲染用 `@Observable` 共享给 `RootView` 的 overlay，别用单例 + 通知的方式（会和 SwiftUI 生命周期打架） |
| 无窗口时的按键限制 | 主窗口关闭后 macOS **不再向应用派发按键**（实测：退出委托回调都不触发） | 因此「仅菜单栏驻留」状态的退出走状态栏面板的「退出」按钮（直接 `terminate`），不要承诺此时 ⌘Q 可用 |
